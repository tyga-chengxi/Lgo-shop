...
import com.lgoshop.dto.BucketPolicyConfigDto;
import com.lgoshop.dto.MinioUploadDto;
import io.minio.*;
...
public class MinioController {
    ...
    @ResponseBody
    public LgoResult upload(@RequestPart("file") MultipartFile file) {
        try {
            //创建一个MinIO的Java客户端
            ...
            return LgoResult.success(minioUploadDto);
        } catch (Exception e) {
            e.printStackTrace();
    ...
    @ResponseBody
    public LgoResult delete(@RequestParam("objectName") String objectName) {
        try {
            MinioClient minioClient = MinioClient.builder()
                    .endpoint(ENDPOINT)
                    .credentials(ACCESS_KEY,SECRET_KEY)
